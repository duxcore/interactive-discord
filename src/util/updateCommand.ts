...
export default function updateCommand(cmd: SlashCommand, client: InteractiveClient): Promise<ApplicationCommand> {
  return new Promise(async (resolve, reject) => {
    ...
    if (isGlobal) {
      ...
      axios.patch(url, cmd.compile(), { headers: { 'Authorization': `Bot ${client.bot.token}` } }).then((dat) => {
        const data = dat.data;
        return resolve(data);
